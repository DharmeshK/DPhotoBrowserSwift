...
public class CaptionView: UIToolbar {
    ...
    init(photo: Photo?) {
        super.init(frame: CGRect(origin: .zero, size: CGSize(width: 320, height: 44))) // Random initial frame
        self.photo = photo
    ...
    private func setupCaption() {
        ...
        label = UILabel(frame: CGRect(
                            x:labelPadding,
                            y:0.0,
                            width:self.bounds.size.width - labelPadding * 2.0,
                            height: self.bounds.size.height).integral)
