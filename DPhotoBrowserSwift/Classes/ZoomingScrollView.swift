...
//  Pods
//
//  Created by DK on 30/05/21 on 06/09/15.
...
public class ZoomingScrollView: UIScrollView, UIScrollViewDelegate, TapDetectingImageViewDelegate, TapDetectingViewDelegate {
    ...
    private var tapView = TapDetectingView(frame: CGRect.zero) // for background taps
    private var photoImageView = TapDetectingImageView(frame: CGRect.zero)
    private var loadingIndicator = DACircularProgressView(frame: CGRect(x:140.0,y: 30.0,width: 40.0,height: 40.0))
    ...
    public init(photoBrowser: PhotoBrowser) {
        ...
        tapView.autoresizingMask = [. flexibleWidth, . flexibleWidth]
        tapView.backgroundColor = UIColor.white
        addSubview(tapView)
        
        // Image view
        photoImageView.tapDelegate = self
        photoImageView.contentMode = UIView.ContentMode.center
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(setProgressFromNotification(notification:)),
            name: .DPhoto_PROGRESS_NOTIFICATION,
            object: nil)
    ...
    var photo: Photo? {
        set(p) {
            ...
            if dPhoto != nil && p == nil {
                dPhoto!.cancelAnyLoading()
            }
            dPhoto = p
            if photoBrowser.imageForPhoto(photo: dPhoto) != nil {
                self.displayImage()
            }
            else {
    ...
    func displayImage() {
        if dPhoto != nil && photoImageView.image == nil {
            // Reset
            ...
            if p.emptyImage {
                if nil == loadingError {
                    ...
                    loadingError!.image = UIImage.imageForResourcePath(
                        path:"DPhotoBrowserSwift.bundle/ImageError",
                        ofType: "png",
    ...
    func setMaxMinZoomScalesForCurrentBounds() {
        ...
        if zoomScale != minScale {
            // Centralise
            contentOffset = CGPoint(x:(imageSize.width * zoomScale - boundsSize.width) / 2.0,
                                    y: (imageSize.height * zoomScale - boundsSize.height) / 2.0)
        }
