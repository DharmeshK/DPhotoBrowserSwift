...
public class GridCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        let videoIndicatorImage = UIImage.imageForResourcePath(path: "DPhotoBrowserSwift.bundle/VideoOverlay",
                                                               ofType: "png",
        ...
        selectedButton.setImage(
            UIImage.imageForResourcePath(
                path: "DPhotoBrowserSwift.bundle/ImageSelectedSmallOff",
                ofType: "png",
                inBundle: Bundle(for: GridCell.self)),
            for: .normal)
        
        selectedButton.setImage(UIImage.imageForResourcePath(
                                    path: "DPhotoBrowserSwift.bundle/ImageSelectedSmallOn",
                                    ofType: "png",
                                    inBundle: Bundle(for: GridCell.self)),
                                for: .selected)
    ...
    @objc public func setProgressFromNotification(notification: Notification) {
        if let dict = notification.object as? [String : AnyObject?],
           ...
           let mwp = dPhoto, photosEqual(p1: photoWithProgress, p2: mwp)
        {
    ...
    @objc public func handlePhotoLoadingDidEndNotification(notification: Notification) {
        if let p = notification.object as? Photo,
           let mwp = dPhoto, photosEqual(p1: p, p2: mwp)
        {
    ...
    private func photosEqual(p1: Photo, p2: Photo) -> Bool {
        return
