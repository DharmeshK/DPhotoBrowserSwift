...
public class PhotoBrowser: UIViewController, UIScrollViewDelegate, UIActionSheetDelegate {
    ...
    // Video
    var currentVideoPlayerViewController: AVPlayerViewController?
    ...
    private func initialisation() {
        ...
        // Listen for DPhoto falsetifications
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePhotoLoadingDidEndNotification(notification:)),
            name: .DPhoto_LOADING_DID_END_NOTIFICATION,
            object: nil)
    }
    ...
    public override dynamic func viewDidLoad() {
        ...
        pagingScrollView.autoresizingMask = [. flexibleWidth, . flexibleWidth]
        ...
        // Toolbar Items
        if displayNavArrows {
            let arrowPathFormat = "DPhotoBrowserSwift.bundle/UIBarButtonItemArrow"
        ...
        if displayActionButton {
            actionButton = UIBarButtonItem(
                ...
                target: self,
                action: #selector(actionButtonPressed(sender:)))
        }
        ...
        if enableSwipeToDismiss {
            let swipeGesture = UISwipeGestureRecognizer(target: self, action: #selector(doneButtonPressed(sender:)))
    ...
    func performLayout() {
        ...
        if let navi = navigationController {
                ...
                navi.viewControllers[0] == self
            {
                ...
                if let done = doneButton {
                    ...
                    done.setTitleTextAttributes([NSAttributedString.Key : Any](), for: .normal)
                    done.setTitleTextAttributes([NSAttributedString.Key : Any](), for: .highlighted)
                   ...
                   let previousViewController = navi.viewControllers[navi.viewControllers.count - 2] as? UINavigationController
                {
                    ...
                    newBackButton.setTitleTextAttributes([NSAttributedString.Key : Any](), for: .normal)
                    newBackButton.setTitleTextAttributes([NSAttributedString.Key : Any](), for: .highlighted)
        ...
        if enableGrid {
            ...
            items.append(UIBarButtonItem(
                            image: UIImage.imageForResourcePath(
                                path:"DPhotoBrowserSwift.bundle/UIBarButtonItemGrid",
                                ofType: "png",
                                inBundle: Bundle(for: PhotoBrowser.self)),
                            style: .plain,
                            target: self,
                ...
                if pageIndex > 0 {
                    ...
                    if let photo = photoAtIndex(index: pageIndex - 1) {
                        if nil == photo.underlyingImage {
                            ...
                            //DLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1)
                        }
                    }
                }
    ...
    //MARK: - DPhoto Loading falsetification
    
    @objc func handlePhotoLoadingDidEndNotification(notification: Notification) {
    ...
    func tilePages() {
        ...
        for page in visiblePages {
            ...
            if pageIndex < iFirstIndex || pageIndex > iLastIndex {
                ...
                //DLog(@"Removed page at index %lu", (unsigned long)pageIndex)
            }
        }
        ...
        for index in iFirstIndex...iLastIndex {
            if !isDisplayingPageForIndex(index: index) {
                ...
                // DLog(@"Added page at index %lu", (unsigned long)index)
                ...
                if page.displayingVideo() {
                    ...
                    playButton.setImage(UIImage.imageForResourcePath(
                                            path: "DPhotoBrowserSwift.bundle/PlayButtonOverlayLarge",
                                            ofType: "png",
                                            inBundle: Bundle(for: PhotoBrowser.self)), for: .normal)
                    
                    playButton.setImage(UIImage.imageForResourcePath(
                                            path: "DPhotoBrowserSwift.bundle/PlayButtonOverlayLargeTap",
                                            ofType: "png",
                ...
                if self.displaySelectionButtons {
                    ...
                    selectedButton.setImage(UIImage.imageForResourcePath(
                                                path:"DPhotoBrowserSwift.bundle/ImageSelectedOff",
                                                ofType: "png",
                                                inBundle: Bundle(for: PhotoBrowser.self)),
                                            for: .normal)
                    
                    let selectedOnImage: UIImage?
                    if customImageSelectedIconName.count > 0 {
                        selectedOnImage = UIImage(named: customImageSelectedIconName)
                    }
                    else {
                        selectedOnImage = UIImage.imageForResourcePath(
                            path:"DPhotoBrowserSwift.bundle/ImageSelectedOn",
                            ofType: "png",
                            inBundle: Bundle(for: PhotoBrowser.self))
                    }
            ...
            if index - 2 >= 0 {
                for i in 0...(index - 2) {
                    if let photo = photos[i] {
                        ...
                        //DLog.log("Released underlying image at index \(i)")
                    }
                }
            }
        }
    ...
    @objc func selectedButtonTapped(sender: AnyObject) {
    ...
    func playVideo(videoURL: URL, atPhotoIndex index: Int) {
        ...
        if let videoPlayer = currentVideoPlayerViewController {
            self.present(videoPlayer, animated: true) {
                let gesture = UIPanGestureRecognizer(target: self, action: nil)
                videoPlayer.view.addGestureRecognizer(gesture)
        ...
        NotificationCenter.default.removeObserver(self,
                                                  ...
                                                  object: currentVideoPlayerViewController!.player?.currentItem)
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(videoFinishedCallback(_:)),
                                               name: NSNotification.Name.AVPlayerItemDidPlayToEndTime,
                                               object: currentVideoPlayerViewController!.player!.currentItem)
    }
    ...
    func clearCurrentVideo() {
        if let videoPlayer = currentVideoPlayerViewController {
            ...
            self.pageDisplayedAtIndex(index: currentVideoIndex)?.playButton?.isHidden = false
    ...
    func setVideoLoadingIndicatorVisible(visible: Bool, atPageIndex: Int) {
        ...
        if nil == currentVideoLoadingIndicator && visible {
            ...
            positionVideoLoadingIndicator()
            self.pageDisplayedAtIndex(index: currentVideoIndex)?.playButton?.isHidden = true
        }
    }
    ...
    @objc func actionButtonPressed(sender: AnyObject) {
...
extension PhotoBrowser : AVPlayerViewControllerDelegate {
